...
import { watchLaterService, WatchLater } from '@/services/watchLaterService';
import { authService } from '@/services/authService';
import { FiTrash2, FiClock, FiFilm, FiBook } from 'react-icons/fi';
...
export default function WatchLaterPage() {
  ...
  const loadWatchLater = async () => {
    try {
      setLoading(true);
      const response = await watchLaterService.getWatchLater('movie', currentPage, 20);
  ...
  const handleClear = async () => {
    ...
    try {
      await watchLaterService.clearWatchLater('movie');
      setWatchLater([]);
    } catch (error) {
